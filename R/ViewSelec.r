...
ViewSelec <- function(SelecParam) {
	...
	smallp <- 0.01
	SelecParam$left <- SelecParam$MtL50Small - log((1-smallp)/smallp)*SelecParam$MtSlopeSmall
	SelecParam$left[SelecParam$left < 0] <- 0
	SelecParam$right <- SelecParam$MtL50Large - log((1-smallp)/smallp)*(-SelecParam$MtSlopeLarge)
	...
	for(z in 1:length(suz)) {
		...
		for(g in 1:length(sug)) {
			...
			y <- logit2(x, both$MtL50Small[sel2], both$MtSlopeSmall[sel2], both$MtL50Large[sel2], -both$MtSlopeLarge[sel2])
