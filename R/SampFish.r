...
SampFish <- function(SimPop, NumEvents=1, AcNum, AcInterval, AcLayer, AcAngle, MtNum, MtHt, MtWd, MtLen, MtMinCat=2, MtMulti=6, 
	...
	AC$interval <- intmids[cut(AC$f.east, include.lowest=TRUE, breaks=intbrks, labels=FALSE)]

	laybrks <- seq(0, SimPop$LakeInfo$BotDepMax + AcLayer - 1, AcLayer)
	laymids <- laybrks[-1] - AcLayer/2
	AC$layer <- laymids[cut(AC$f.wdep, include.lowest=TRUE, breaks=laybrks, labels=FALSE)]
	...
	ac.out.vars <- c("Event", "ACid", "ACnorth", "interval", "layer", "f.east", "f.north", "f.d2sh", "f.botdep", "f.wdep", "f.d2bot", "ts", "G", "len", "wt")
	# setdiff(names(AC), ac.out.vars)
	# 3 variables unknown to targets: "G" "len" "wt"
	mtr.out.vars <- c("Event", "ACid", "ACnorth", "MTRid", "MTReast", "MTRd2sh", "MTRbdep", "MTRwdep", "MTRd2bot", "G", "len", "wt",
	...
	SurvParam <- c(NumEvents=NumEvents, AcNum=AcNum, AcInterval=AcInterval, AcLayer=AcLayer, AcAngle=AcAngle, 
		MtNum=MtNum, MtHt=MtHt, MtWd=MtWd, MtLen=MtLen, MtMinCat=MtMinCat, MtMulti=MtMulti, Seed=Seed)
